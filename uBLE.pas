...
procedure StartDirectedAdvertising;   // page 137 v4,2
procedure StartPassiveScanning;       // page 139 v4.2
procedure StartActiveScanning;        // page 140 v4.2
...
// helper functions
procedure SetBeaconEvent (anEvent : TBTBeaconEvent);
...
procedure StartDirectedAdvertising;
begin
  //
end;
...
begin
  SetLEScanParameters (LL_SCAN_PASSIVE, 2000, 1000, $00, $00);
  SetLEScanEnable (true, false);
end;

procedure StartActiveScanning;
begin
  SetLEScanParameters (LL_SCAN_ACTIVE, 2000, 1000, $00, $00);
  SetLEScanEnable (true, false);
end;
...
begin
  ...
  AdData[Len + 1] := Type_;
  for i := 0 to high (Data) do AdData[Len + 2 + i] := Data[i];
end;
...
begin
  ...
  AdData[Len] := Length (Data) + 1;
  AdData[Len + 1] := Type_;
  for i := 1 to length (Data) do AdData[Len + 1 + i] := ord (Data[i]);
end;
